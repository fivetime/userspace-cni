...
type MemifConf struct {
	...
	// Optional performance/tuning knobs passed through to VPP's memif_create_v2.
	// Zero values mean "use VPP defaults" (RxQueues/TxQueues=1, RingSize=1024,
	// BufferSize=2048). These are VPP-only and ignored by the ovs-dpdk engine.
	RxQueues   uint8  `json:"rxQueues,omitempty"`   // number of rx queues (multi-queue)
	TxQueues   uint8  `json:"txQueues,omitempty"`   // number of tx queues (multi-queue)
	RingSize   uint32 `json:"ringSize,omitempty"`   // ring size, must be a power of 2
	BufferSize uint16 `json:"bufferSize,omitempty"` // packet buffer size in bytes
	Secret     string `json:"secret,omitempty"`     // optional shared secret, max 24 chars
	NoZeroCopy bool   `json:"noZeroCopy,omitempty"` // disable zero-copy mode
	UseDma     bool   `json:"useDma,omitempty"`     // enable DMA acceleration
...
type VhostConf struct {
	...
	// VPP vhost-user tuning passed through to create_vhost_user_if_v2. These are
	// VPP-only and ignored by the ovs-dpdk engine (which only uses Mode).
	EnableGso      bool `json:"enableGso,omitempty"`      // generic segmentation offload
	EnablePacked   bool `json:"enablePacked,omitempty"`   // packed virtqueue format
	EnableEventIdx bool `json:"enableEventIdx,omitempty"` // virtio event index

	// OVS-DPDK per-port ingress rate limiting (QoS). Ignored by VPP.
	// 0 = no limit. Rate is in kbps, Burst in kb.
	IngressPolicingRate  int `json:"ingressPolicingRate,omitempty"`
	IngressPolicingBurst int `json:"ingressPolicingBurst,omitempty"`
...
type BridgeConf struct {
	...
	VlanId     int    `json:"vlanId,omitempty"`     // Optional VLAN Id (access tag, or native VLAN with trunks)

	// OVS-DPDK VLAN trunking (ignored by VPP). VlanMode is one of
	// access|trunk|native-tagged|native-untagged|dot1q-tunnel; when empty OVS
	// infers access (tag) or trunk (trunks) from which of VlanId/Trunks is set.
	Trunks   []int  `json:"trunks,omitempty"`   // allowed VLAN ids for a trunk port
	VlanMode string `json:"vlanMode,omitempty"` // explicit OVS Port vlan_mode
}
...
type UserSpaceConf struct {
	...
	MTU        int        `json:"mtu,omitempty"`     // Optional interface MTU; 0 = leave engine default
	MAC        string     `json:"mac,omitempty"`     // Optional fixed MAC; empty = annotation MAC (OVS) or random
...
type NetConf struct {
	...
	*/
	...
	// Args carries the CNI "args" passthrough from StdinData. Meta-plugins such
	// as multus-cni propagate per-pod cni-args here as args.cni.<key>; this is
	// how an OvnPort (and MAC) from a pod annotation reaches the plugin.
...
// kept as raw JSON so non-string entries can be skipped without failing.
